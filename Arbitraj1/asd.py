...
def run():
    print("BINANCE ARBITRAJ BOTUNA HOSGELDINIZ. BOT BIRAZDAN BASLIYACAK.")
    time.sleep(3)
    while True:
        initialize_arb()
...
def initialize_arb():
    ...
    list_of_arb_sym = [list_of_symbols2,list_of_symbols5,list_of_symbols6
    ...
    portfolio=[]
    with open('C:/Users/Monster/Desktop/Arbitraj2/CryptoBot/binanceArbitrage/Portfolio.txt','r') as f1:
    ...
    for arb_market in list_of_arb_sym:
        # a = str(arb_market[0])
        # if (str(arb_market[0]) in listA) & (str(arb_market[1]) in listA) & (str(arb_market[2]) in listA):
        calc_profit_list.append(arbitrage_bin(arb_market, tickers, portfolio, 1, 1,client))
...
def arbitrage_bin(list_of_sym, tickers, portfolio, cycle_num=1, cycle_time=30, place_order='No', real_order='No'):
    print("Binance Arbitraj Botu Calisiyor\n")
    fee_percentage = 0.1*3          #divided by 100
    for i in range(0,1):   
        print("Karsilastirilan Semboller :", list_of_sym)
        list_exch_rate_list = []
        if 1:
